...
public sealed class EfProjectStore(IDbContextFactory<AppDbContext> dbContextFactory) : IProjectStore
{
    ...
    public async Task<WorkspaceCleanupResult?> TryCleanupWorkspaceIfEmptyAsync(
        Guid workspaceId,
        CancellationToken cancellationToken)
    {
        await using AppDbContext db = await dbContextFactory.CreateDbContextAsync(cancellationToken);
        ...
        Guid projectId = workspace.ProjectId;
        int workspaceCount = await db.Workspaces.CountAsync(
        ...
        if (workspaceCount <= 1)
        {
            ...
            List<Chat> orphanChats = await db.Chats
                ...
                .Where(chat =>
                    ...
                    || (chat.WorkspaceId != null && workspaceIds.Contains(chat.WorkspaceId.Value)))
                .ToListAsync(cancellationToken);
            ...
            Project? project = await db.Projects.FirstOrDefaultAsync(
                existing => existing.Id == projectId,
                cancellationToken);
            ...
            db.Projects.Remove(project);
            await db.SaveChangesAsync(cancellationToken);
            return new WorkspaceCleanupResult(projectId, workspaceId, orphanChats.Select(chat => chat.Id).ToList());
        }

        List<Chat> workspaceOrphanChats = await db.Chats
            .IgnoreQueryFilters()
            .Where(chat => chat.WorkspaceId == workspaceId)
            .ToListAsync(cancellationToken);

        foreach (Chat chat in workspaceOrphanChats)
        {
            chat.WorkspaceId = null;
        }
        ...
        db.Workspaces.Remove(workspace);
        await db.SaveChangesAsync(cancellationToken);
        return new WorkspaceCleanupResult(null, workspaceId, workspaceOrphanChats.Select(chat => chat.Id).ToList());
    }
}
