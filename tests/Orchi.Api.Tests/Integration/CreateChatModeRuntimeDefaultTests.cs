...
public class CreateChatModeRuntimeDefaultTests : IClassFixture<TestWebApplicationFactory>, IAsyncLifetime
{
    private readonly TestWebApplicationFactory _factory;
    private readonly HttpClient _client;

    public CreateChatModeRuntimeDefaultTests(TestWebApplicationFactory factory)
    {
        _factory = factory;
    ...
    public async Task KickOffPlan_ChildInheritsParentModel()
    {
        ...
        await _client.PatchAsJsonAsync(
            $"/agents/mode-defaults/{AgentModeIds.Implementation}",
            new UpdateModeRuntimeDefault.Request("cursor", "gpt-5.3-codex", null, null, null));
    ...
    public async Task KickOffReview_ChildInheritsParentModel()
    {
        ...
        HttpResponseMessage implementationDefaults = await _client.PatchAsJsonAsync(
            $"/agents/mode-defaults/{AgentModeIds.Implementation}",
            new UpdateModeRuntimeDefault.Request("cursor", "composer-2.5-fast", null, null, null));
        Assert.Equal(HttpStatusCode.OK, implementationDefaults.StatusCode);

        HttpResponseMessage reviewDefaults = await _client.PatchAsJsonAsync(
            $"/agents/mode-defaults/{AgentModeIds.Review}",
            new UpdateModeRuntimeDefault.Request("cursor", "composer-2.5-fast", null, null, null));
        Assert.Equal(HttpStatusCode.OK, reviewDefaults.StatusCode);
