...
public class EfPlanStoreTests
{
    ...
    [Fact]
    public async Task ListBySourceChatAsync_ReturnsPlansForChatOrderedByCreatedAt()
    {
        string databasePath = Path.Combine(Path.GetTempPath(), $"orchi-plan-unit-{Guid.NewGuid():N}.db");
        Guid sourceChatId = Guid.NewGuid();
        ...
        try
        {
            ...
            await store.UpsertAsync(
                new PlanUpsertModel("beta-plan", sourceChatId, "Beta", "# Beta"),
                CancellationToken.None);
            await store.UpsertAsync(
                new PlanUpsertModel("alpha-plan", sourceChatId, "Alpha", "# Alpha"),
                CancellationToken.None);

            IReadOnlyList<StoredPlan> listed =
                await store.ListBySourceChatAsync(sourceChatId, CancellationToken.None);

            Assert.Equal(2, listed.Count);
            Assert.Contains(listed, plan => plan.PlanId == "alpha-plan");
            Assert.Contains(listed, plan => plan.PlanId == "beta-plan");
        }
        finally
        {
            SqliteConnection.ClearAllPools();
